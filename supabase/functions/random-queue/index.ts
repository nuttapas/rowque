...
export default async function (req: Request) {
  try {
    ...
    const { round_id, position } = body;
    if (!round_id || !position) return new Response(JSON.stringify({ success: false, code: 'BAD_REQUEST', message: 'round_id and position required' }), { status: 400 });
